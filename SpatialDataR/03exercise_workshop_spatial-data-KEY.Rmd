...
#let's take a look at the output:
  #what geometry type is this dataset? MULTIPOLYGON
  #what is the CRS? WGS 84
...
#Note - we might notice something strange with the above code! That's because fad_sf has some NA observations that plot to 0,0. Let's get rid of those:
fad_sf <- fad_sf %>%
  filter(!is.na(X))
...
base_map <- ne_coastline(scale = 'large') #read in base map
#remeber to check CRS

ggplot() +
  geom_sf(data = base_map, color = "black") +
  geom_sf(data = streams, color = "skyblue") +
  coord_sf(ylim = c(19, 23), 
           xlim = c(-161, -154)) #limit plot to just hawaii

#Find the closest moku to each FAD (hint: st_nearest_feature)
#first, get nearest moku of each FAD (index)
closest_moku <- st_nearest_feature(fad_sf, moku)

#next, add the name of that moku to the FAD dataframe
fad_sf <- fad_sf %>% 
  mutate(Moku = moku$moku[closest_moku])

#finally, we can plot to take a look!
ggplot() +
  geom_sf(data = base_map, color = "black") +
  geom_sf(data = fad_sf, aes(color = Moku)) +
  coord_sf(ylim = c(19, 23), 
           xlim = c(-161, -154)) #limit plot to just hawaii

#Extract only streams from Maui (hint: st_intersection and maui_moku)

#If the 'streams' object had a column for island, this would be an easy filter(). It doesn't - but we can do something else!

#We can keep only streams that overlap our 'maui_moku' object:
maui_streams <- st_intersection(maui_moku, streams)

#plot to test!
ggplot() +
  geom_sf(data = maui_moku) +
  geom_sf(data = maui_streams)

#Extract only streams aboe a certain length (hint: filter)
long_streams <- streams %>%
  filter(length > 1000)
...
ggplot() +
  geom_sf(data = moku) + #our sf object
  geom_spatraster(data = sst) + #our raster object
  scale_fill_continuous(low="blue", high="darkred", #color scale
                        na.value="transparent")

#Change the resolution of a raster (hint: aggregate or disagg)
effluent_low_res <- aggregate(effluent, fact = 5, fun = "mean")

#Make a histogram of raster values (hint: hist())
hist(oahu_effluent)

#Subtract one raster from another (hint: -)
eff_minus_nitrogen <- oahu_effluent - oahu_nitro
  
#Add two rasters together (hint: +)
nitro_phos <- oahu_nitro + oahu_phos
...
#adding a new column (average) to our moku_spat object
moku_spat <- moku_spat %>%
  mutate(average = extract(sst, ., fun = "mean", na.rm = TRUE)$hi_otp_all_sst_clim_max)

#plot to test!
ggplot(data = moku_spat) + 
  geom_spatvector(aes(fill = average)) +
